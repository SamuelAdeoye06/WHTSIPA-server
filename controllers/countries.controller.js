...
/* ─────────────────────────────────────────────────────────────────
   Complete ISO 3166-1 world country list  (code, name, dial)
   ~250 entries — used to seed the DB on first launch
───────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────
   Seed helper — called once on startup if the collection is empty
───────────────────────────────────────────────────────────────── */
export async function seedCountriesIfEmpty() {
  const count = await CountrySettings.countDocuments()
...
/* ─────────────────────────────────────────────────────────────────
   GET /api/countries
   Public — returns all countries where showInDropdown = true,
   including their signupAllowed flag so the frontend knows which
   subset can sign up.
───────────────────────────────────────────────────────────────── */
export async function getPublicCountries(req, res) {
  try {
    const countries = await CountrySettings
      .find({ showInDropdown: true })
      .select('code name dial signupAllowed -_id')
      .sort({ name: 1 })
    return res.json(countries)
  } catch (err) {
    console.error('getPublicCountries error:', err)
...
/* ─────────────────────────────────────────────────────────────────
   ...
   Admin only — full list including hidden countries.
───────────────────────────────────────────────────────────────── */
export async function getAdminCountries(req, res) {
  try {
    const countries = await CountrySettings
      .find()
    ...
    console.error('getAdminCountries error:', err)
    return res.status(500).json({ message: 'Server error.' })
  }
}
...
export async function patchCountry(req, res) {
  try {
    ...
    const updated = await CountrySettings.findOneAndUpdate(
